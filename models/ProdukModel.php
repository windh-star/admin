...
class ProdukModel extends CI_Model{
    ...
    function getTabelProduk($datatable){
      $columns = implode(', ', $datatable['col-display']);
      $query  = "(SELECT produk.*,bahan.nama_bahan,pengguna.nama_pengguna,sub_kategori_produk.sub_kategori FROM produk,bahan,pengguna,sub_kategori_produk WHERE produk.id_bahan=bahan.id_bahan AND produk.id_pengguna=pengguna.id_pengguna AND produk.id_sub_kategori=sub_kategori_produk.id_sub_kategori) a";
      ...
      foreach ($data->result() as $row) {
         ...
         for ($i=0; $i < $count_c; $i++) {
            $field = $columnd[$i];
            if ($i == 16) $data[] = "Rp ".number_format($row->$field, 2, ",", ".");
            else $data[] = $row->$field;
         }
         $option['data'][] = $data;
      }
